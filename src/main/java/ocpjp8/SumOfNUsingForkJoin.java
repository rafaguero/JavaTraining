...
public class SumOfNUsingForkJoin {
    ...
    static class RecursiveSumOfN extends RecursiveTask<Long> {
        ...
        public Long compute(){
            if((to-from) <= N/NUM_THREADS){
                ...
                long mid = (from + to) / 2;
                System.out.printf("Forking computation into two ranges: " + "%d to %d and %d %n",from,mid,mid,to);
    ...
    public static void main(String[] args) {
        ...
        long computeSum = pool.invoke(new RecursiveSumOfN(0,N));
        long formulaSum = (N*(N+1))/2;
        System.out.printf("Sum for range 1..%d, computed sum = %d, formula sum = %d %n",N,computeSum,formulaSum);
    }
}
